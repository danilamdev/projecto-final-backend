          ...
          {cart.length === 0 && !loading && <h2 className="text-xl mb-5">No tienes productos en tu carrito</h2>}
            {
                  ...
                  <div key={product._id} className="bg-white w-full mt-2 flex items-center overflow-hidden hover:scale-[1.01] transition-all group">
                    ...
                    <div className="flex justify-between w-full">
                      ...
                      <div className="right bg-red-100 text-red-400 px-4 grid place-content-center translate-x-24 group-hover:translate-x-0 transition-transform">
                        <button onClick={() => handleRemove2(product._id)} >Remove</button>
                      </div>
                    </div>
                  </div>
                )
             ...
             <div className="bg-lime-50/50 w-full mt-2 flex items-center overflow-hidden hover:scale-[1.01] transition-all group"
             onClick={calcularTotal} >
                <div className="h-full bg-lime-200 w-2"></div>
                <div className="flex justify-between w-full">
                ...
                {cart.length === 0 
                  ? null
                  : <button onClick={handleCheckout} className="bg-blue-400 mt-5 py-5 font-bold text-xl rounded-lg text-white uppercase">comprar</button>
                  }
        </div>
        : null
      }
